...
public enum RenderMathUtils {
	...
	public static ByteBuffer wrapFaces(final int[] faces) {
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(faces.length * 2).order(ByteOrder.nativeOrder());
		final ShortBuffer wrapper = byteBuffer
				.asShortBuffer();
		for (final int face : faces) {
			wrapper.put((short) (face & 0xFFFF));
		}
		wrapper.clear();
		return byteBuffer;
	}
	...
	public static ByteBuffer wrap(final float[] positions) {
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(positions.length * 4).order(ByteOrder.nativeOrder());
		final FloatBuffer wrapper = byteBuffer
				.asFloatBuffer();
	...
	public static ByteBuffer wrap(final int[] positions) {
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(positions.length * 4).order(ByteOrder.nativeOrder());
		final IntBuffer wrapper = byteBuffer
				.asIntBuffer();
		wrapper.put(positions);
		wrapper.clear();
		return byteBuffer;
	...
	public static Buffer wrapPairs(final float[][] quadVertices) {
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(quadVertices.length * 8).order(ByteOrder.nativeOrder());
		final FloatBuffer wrapper = byteBuffer
				.asFloatBuffer();
	...
	public static Buffer wrap(final int[][] quadIndices) {
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(quadIndices.length * 3 * 4).order(ByteOrder.nativeOrder());
		final IntBuffer wrapper = byteBuffer
				.asIntBuffer();
	...
	public static Buffer wrap(final List<float[]> vertices) {
		...
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(vertices.size() * expectedNumberOfFloats * 4)
				.order(ByteOrder.nativeOrder());
		final FloatBuffer wrapper = byteBuffer.asFloatBuffer();
	...
	public static Buffer wrapFaces(final List<int[]> indices) {
		...
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(indices.size() * expectedNumberOfValues * 2)
				.order(ByteOrder.nativeOrder());
		final ShortBuffer wrapper = byteBuffer.asShortBuffer();
